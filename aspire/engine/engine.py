...
CLI（子进程隔离执行，供 harness 调用）：
    MUJOCO_GL=egl python -m aspire.engine.engine --code task_code.py --task Lift --seed 0
"""
...
from ..api.primitives import build_namespace
from ..evidence.trace import Tracer
...
class ExecutionEngine:
    ...
    def run(self, code: str, code_ref: str | None = None) -> dict:
        ...
        try:
            if self.render:
                # 可视化模式（2026-08-06 用户反馈"机械臂不动"——实为 IK/RRT 规划
                # 阶段分钟级静默的感知问题）: 任务代码 print 实时回显终端（tee），
                # 同时照常捕获入 trace；headless 路径行为不变。
                class _Tee(io.StringIO):
                    def write(self, s):
                        sys.__stdout__.write(s)
                        sys.__stdout__.flush()
                        return buf.write(s)
                _ctx = contextlib.redirect_stdout(_Tee())
            else:
