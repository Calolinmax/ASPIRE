...
> CGN 抓取管线端到端 PASS。详细进度看板见 [docs/api_asset_map.md](docs/api_asset_map.md)。
...
│  任务层   scripts/ （demo_stack_piper_capx / cgn_execute_grasp / ...）                          │
│  API 层   aspire/primitives_capx.py + engine_capx.py   （PiperControlApiReduced, 15 函数）      │
│  执行层   aspire/engine.py + trace.py                  （执行引擎 + trace 记录）                │
│  感知     aspire/vision_sam3.py     → SAM3 本地权重（transformers, GPU ~8GB）                   │
│  抓取     aspire/vision_client.py   → HTTP → CGN Docker 容器（见下）                            │
│  控制     aspire/pyroki_client.py   → pyroki IK 服务 ／ aspire/motion_planner.py (RRT-Connect)  │
│  仿真     robosuite 1.5.2 + MuJoCo 3.3.7 + aspire/robots/（Piper MJCF + 37k 条 IK 种子库）      │
└───────────────────────────────────────────────────────────────────────────────────────────────┘
...
| 路径 | 说明 |
|---|---|
| `aspire/` | 核心包：engine / primitives(_capx) / vision_sam3 / vision_client / pyroki_client / motion_planner / trace / robots |
| `scripts/` | 任务脚本与工具：demo、标定、IK 验证、自检（入口见 §6） |
| `docs/` | 设计与运维文档（CGN 容器、SAM3、API 对照表等） |
...
python scripts/test_piper_build.py

# API 全量自检（15 函数契约）
python scripts/test_piper_capx_api.py
...
| 脚本 | 说明 |
|---|---|
| `scripts/demo_stack_piper_capx.py` | Stack 任务 demo（SAM3 引导 + cap-x API 线） |
| `scripts/cgn_execute_grasp.py` | CGN 抓取端到端 |
| `scripts/demo_stack_dualcam.py` | 双相机 Stack 流程 |

更多标定/诊断/可视化脚本见 `scripts/` 目录与 `docs/` 对应文档。

## 7. 文档索引

- [ASPIRE_REPRO_PLAN.md](ASPIRE_REPRO_PLAN.md) — 长期复现规划与现状基线
